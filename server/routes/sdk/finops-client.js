...
class FinOpsClient {
  ...
  // Non-streaming chat completion. `provider` is 'openai' or 'anthropic'.
  // `body` is the exact provider-format request body (model, messages, etc.)
  ...
  // Streaming chat completion - returns an async generator yielding raw SSE
  // text chunks as they arrive. Caching does not apply to streamed requests
  // (see server/routes/proxy.js for why).
