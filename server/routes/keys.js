...
=======
  ...
  const rows = tenancy.MULTI_TENANT
    ? await req.controlPlaneDb.all(
        "SELECT id, key_id, label, role, team, status, quarantine_reason, created_at FROM api_keys WHERE tenant_id = ? ORDER BY id DESC",
        [req.tenantId]
      )
    : await req.controlPlaneDb.all(
...
router.post("/", requireAuth("manage_keys"), async (req, res) => {
  ...
  if (tenancy.MULTI_TENANT) {
    const created = await tenancy.createTenantApiKey({ tenant_id: req.tenantId, label, role });
    await logAudit(req.apiKey.key_id, "key.create", created.key_id, { role, team: team || null }, req.db);
    return res.status(201).json({ ...created, team: team || null });
  }
...
=======
  await req.controlPlaneDb.run("INSERT INTO api_keys (key_id, label, role, team) VALUES (?, ?, ?, ?)", [
>>>>>>> theirs
    key_id,
    label,
    role,
