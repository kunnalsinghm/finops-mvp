...
async function seedEvent({ team = null, environment = null, provider = "openai", model = "gpt-4o-mini", cost_usd, input_tokens = 0, output_tokens = 0, tagged = 1, event_time }) {
  await storage.run(
    `INSERT INTO usage_events (event_time, provider, model, team, environment, cost_usd, input_tokens, output_tokens, tagged)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [event_time || new Date().toISOString(), provider, model, team, environment, cost_usd, input_tokens, output_tokens, tagged]
  );
}
